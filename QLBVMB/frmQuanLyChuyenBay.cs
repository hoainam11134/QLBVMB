...
namespace Plane_Ticket
{
    public partial class frmQuanLyChuyenBay : Form
    {
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            int thoiGianBayToiThieu = Convert.ToInt32(row["THOIGIANBAYTOITHIEU"]);
            ...
            if (cboMaTuyenBay.Text.Trim() != "" && cboMaMayBay.Text.Trim() != "" && txtThoiGianBay.Text.Trim()!="")
            {
                try
                {
                    ...
                    if (Convert.ToInt32(txtThoiGianBay.Text) > thoiGianBayToiThieu && busChuyenBay.Add(dtoChuyenBay))
                    {
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            DataRow row = dtThamSo.Rows[0];
            int thoiGianBayToiThieu = Convert.ToInt32(row["THOIGIANBAYTOITHIEU"]);
            int thoiGianDungToiThieu = Convert.ToInt32(row["THOIGIANDUNGTOITHIEU"]);
            int thoiGianDungToiDa = Convert.ToInt32(row["THOIGIANDUNGTOIDA"]);
            if (txtMaChuyenBay.Text.Trim() != "")
            {
                if (cboMaTuyenBay.Text.Trim() != "" && cboMaMayBay.Text.Trim() != "" && txtThoiGianBay.Text.Trim() != "")
                {
                    ...
                    {
                        dtoChuyenBay = new DTO_ChuyenBay(txtMaChuyenBay.Text, cboMaTuyenBay.Text, cboMaMayBay.Text, dtpkThoiGianKhoiHanh.Value, float.Parse(txtThoiGianBay.Text));
                        if (Convert.ToInt32(txtThoiGianBay.Text) >= thoiGianBayToiThieu && Convert.ToInt32(txtThoiGianNghi.Text) >= thoiGianDungToiThieu && Convert.ToInt32(txtThoiGianNghi.Text) <= thoiGianDungToiDa && busChuyenBay.Update(dtoChuyenBay))
        ...
        {
            BUS_MayBay busMayBay = new BUS_MayBay();
            DataTable dtMayBay = busMayBay.GetOfMaMayBay(cboMaMayBay.Text);
            if (dtMayBay.Rows.Count == 0)
                return;
            DataRow row = dtMayBay.Rows[0];
            int soGheTong = Convert.ToInt32(row["SOLUONGGHE"]);
            if (cboMaHangVe.Text!="" && txtTongSoGhe.Text!="")
            {      
                try
                {             
                    dtoTinhTrangVe = new DTO_TinhTrangVe(txtMaChuyenBay.Text, cboMaHangVe.SelectedValue.ToString(), Convert.ToInt32(txtTongSoGhe.Text));
                    if(Convert.ToInt32(txtTongSoGhe.Text) < soGheTong && busTinhTrangVe.Add(dtoTinhTrangVe))
                    {
                        int soGheConLai = soGheTong - Convert.ToInt32(txtTongSoGhe.Text);
                        if(cboMaHangVe.SelectedValue.ToString() == "0")
                        {
                            DTO_TinhTrangVe dtoTinhTrangVe2 = new DTO_TinhTrangVe(txtMaChuyenBay.Text, "1", soGheConLai, soGheConLai);
                            busTinhTrangVe.Add(dtoTinhTrangVe2);
                        }
                        else if(cboMaHangVe.SelectedValue.ToString() == "1")
                        {
                            DTO_TinhTrangVe dtoTinhTrangVe3 = new DTO_TinhTrangVe(txtMaChuyenBay.Text, "0", soGheConLai, soGheConLai);
                            busTinhTrangVe.Add(dtoTinhTrangVe3);
                        }
                        MessageBox.Show("Thêm thành công!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
        ...
        private void btnSuaHV_Click(object sender, EventArgs e)
        {
            ...
            string soGheTrong = busTinhTrangVe.GetSoGheTrongOfMaChuyenBayAndMaHangVe(txtMaChuyenBay.Text, cboMaHangVe.SelectedValue.ToString());

            BUS_TinhTrangVe busTinhTrangVe2 = new BUS_TinhTrangVe();
            string tongSoGhe = busTinhTrangVe.GetSoGheTrongOfMaChuyenBayAndMaHangVe(txtMaChuyenBay.Text, cboMaHangVe.SelectedValue.ToString());
            if (flagHangVeCellClick)
            {
                if (cboMaHangVe.Text != "" && txtTongSoGhe.Text != "")
                {
                    try
                    {
                        ...
                        if (Convert.ToInt32(txtTongSoGhe.Text) < Convert.ToInt32(soGheTrong) && busTinhTrangVe.Update(dtoTinhTrangVe))
                        {
                            ...
                            {
                                DTO_TinhTrangVe dtoTinhTrangVe2 = new DTO_TinhTrangVe(txtMaChuyenBay.Text, "1", soGheConLai, soGheConLai);
                                busTinhTrangVe.Update(dtoTinhTrangVe2);
                            }
                            else if (cboMaHangVe.SelectedValue.ToString() == "1")
                            {
                                DTO_TinhTrangVe dtoTinhTrangVe3 = new DTO_TinhTrangVe(txtMaChuyenBay.Text, "0", soGheConLai, soGheConLai);
                                busTinhTrangVe.Update(dtoTinhTrangVe3);
                            }

                            MessageBox.Show("Sửa thành công!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        else
                            MessageBox.Show("Sửa không thành công! Có thể chuyến bay đã có vé bán ra !", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
        ...
        private void btnThemSBTG_Click(object sender, EventArgs e)
        {
            ...
            if (cboTenSanBayTG.Text!="" && txtThoiGianNghi.Text!="" && txtGhiChu.Text!="")
            {
                try
                {
                    ...
                    if (Convert.ToInt32(txtThoiGianNghi.Text) >= thoiGianDungToiThieu && Convert.ToInt32(txtThoiGianNghi.Text) <= thoiGianDungToiDa && busCTChuyenBay.Add(dtoCTChuyenBay))
                        MessageBox.Show("Thêm thành công!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    else
                        MessageBox.Show("Thêm không thành công!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void btnSuaSBTG_Click(object sender, EventArgs e)
        {
            if (flagSBTGCellClick)
            {
                if (cboTenSanBayTG.Text != "" && txtThoiGianNghi.Text != "" && txtGhiChu.Text != "")
                {
                    ...
                    {
                        dtoCTChuyenBay = new DTO_CTChuyenBay(txtMaChuyenBay.Text, cboTenSanBayTG.SelectedValue.ToString(), float.Parse(txtThoiGianNghi.Text), txtGhiChu.Text);
                        if (Convert.ToInt32(txtThoiGianNghi.Text) >= 10 && Convert.ToInt32(txtThoiGianNghi.Text) <= 20 &&  busCTChuyenBay.Update(dtoCTChuyenBay))
                            MessageBox.Show("Sửa thành công!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        else
                            MessageBox.Show("Sửa không thành công!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
        ...
        private void txtThoiGianBay_Validating(object sender, CancelEventArgs e)
        {
            BUS_ThamSo busThamSo = new BUS_ThamSo();
            DataTable dtThamSo = busThamSo.Get();
            if (dtThamSo.Rows.Count == 0)
            ...
            {
                if(Convert.ToInt32(txtThoiGianBay.Text) < thoiGianBayToiThieu)
                {          
                    e.Cancel = true;
                    txtThoiGianBay.Focus();
                    errorProvider1.SetError(txtThoiGianBay, "Thời gian bay quá ít, vui lòng nhập lớn hơn "+ thoiGianBayToiThieu + " phút!!!");
                }
                else
                {
                    e.Cancel = false;
                    errorProvider1.SetError(txtThoiGianBay, "");
                }
            }
            else
            {
                errorProvider1.SetError(txtThoiGianBay, "Vui lòng nhập đầy đủ thông tin !");
            }
        }

        private void txtThoiGianNghi_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private void txtThoiGianNghi_Validating(object sender, CancelEventArgs e)
        {
            BUS_ThamSo busThamSo = new BUS_ThamSo();
            DataTable dtThamSo = busThamSo.Get();
            if (dtThamSo.Rows.Count == 0)
            ...
            int thoiGianDungToiThieu = Convert.ToInt32(row["THOIGIANDUNGTOITHIEU"]);
            int thoiGianDungToiDa = Convert.ToInt32(row["THOIGIANDUNGTOIDA"]);
            if (txtThoiGianNghi.Text != "")
            {
                if (Convert.ToInt32(txtThoiGianNghi.Text) <= thoiGianDungToiThieu || Convert.ToInt32(txtThoiGianNghi.Text) >= thoiGianDungToiDa)
                {
                    e.Cancel = true;
                    txtThoiGianNghi.Focus();
                    errorProvider2.SetError(txtThoiGianNghi, "Thời gian nghỉ chỉ từ "+ thoiGianDungToiThieu + " đến " + thoiGianDungToiDa + " phút !!!");
                }
                else
                {
                    e.Cancel = false;
                    errorProvider2.SetError(txtThoiGianNghi, "");
                }
            }
            else
            {
                errorProvider2.SetError(txtThoiGianNghi, "Vui lòng nhập đầy đủ thông tin");
            }
        }

        private void txtThoiGianBay_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (!char.IsControl(e.KeyChar) && !char.IsDigit(e.KeyChar))
            {
                e.Handled = true;
        ...
        private void txtTongSoGhe_Validating(object sender, CancelEventArgs e)
        {
            BUS_MayBay busMayBay = new BUS_MayBay();
            DataTable dtMayBay = busMayBay.GetOfMaMayBay(cboMaMayBay.Text);
            if (dtMayBay.Rows.Count == 0)
            ...
            {
                if (Convert.ToInt32(txtTongSoGhe.Text) > soGheTong)
                {
                    e.Cancel = true;
                    txtTongSoGhe.Focus();
                    errorProvider3.SetError(txtTongSoGhe, "Tổng số ghế vượt quá giới hạn tổng: " + soGheTong + "! Vui lòng nhập nhỏ hơn");
                }
                else
                {
                    e.Cancel = false;
                    errorProvider3.SetError(txtTongSoGhe, "");
                }
            }
            else
            {
                errorProvider3.SetError(txtTongSoGhe, "Vui lòng nhập đầy đủ thông tin");
            }
        }
    }
}
